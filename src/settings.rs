...
impl SettingsTab {
	...
	pub fn update(&mut self, message: SettingsMessage) -> Task<Message> {
		match message {
			...
			SettingsMessage::SearchEnginePressed => Task::perform(Self::open_engine_exe(), Message::EngineFileChosen),
			...
			SettingsMessage::ChangePressed => {
				let engine_path = if self.engine_path.is_empty() { None } else { Some(self.engine_path.clone()) };
	...
	pub async fn send_changes(
		play_sound: bool, auto_load: bool, flip: bool, coords: bool, pieces: styles::PieceTheme, theme: styles::BoardTheme, engine: String,
		lang: lang::Language,
	) -> Option<config::OfflinePuzzlesConfig> {
		let engine = if engine.is_empty() { None } else { Some(engine) };
		let mut config = config::load_config();
	...
	fn content(&self) -> Element<'_, Message> {
		let col_settings = column![
			row![
				...
				PickList::new(&styles::PieceTheme::ALL[..], Some(self.piece_theme), SettingsMessage::SelectPieceTheme)
			...
			row![
				...
				PickList::new(&styles::BoardTheme::ALL[..], Some(self.board_theme), SettingsMessage::SelectBoardTheme)
					.style(styles::pick_list_style)
					.menu_style(styles::menu_style)
			]
			.spacing(5)
			...
			row![
				...
				PickList::new(PickListWrapper::get_langs(self.lang.lang), Some(self.lang.clone()), SettingsMessage::SelectLanguage)
					.style(styles::pick_list_style)
					.menu_style(styles::menu_style)
			]
			.spacing(5)
			...
			row![
				...
				TextInput::new(&self.export_pgs, &self.export_pgs,).on_input(SettingsMessage::ChangePDFExportPgs).width(60),
			]
			.spacing(5)
			.align_y(Alignment::Center),
			row![
				Text::new(lang::tr(&self.lang.lang, "get_first_puzzles1")),
				TextInput::new(&self.search_results_limit_value, &self.search_results_limit_value,)
			...
			row![
				TextInput::new(&self.engine_path, &self.engine_path,).on_input(SettingsMessage::ChangeEnginePath).width(200),
		...
		let content: Element<SettingsMessage, Theme, iced::Renderer> =
			Container::new(Scrollable::new(Column::new().padding([0, 30]).spacing(10).push(col_settings)))
